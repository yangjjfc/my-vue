<template>
    <Row class="container">
        ...
        <Col span="10" class="logo" :class="collapsed?'logo-collapse-width':'logo-width'"> {{collapsed?'':sysName}}
        </Col>
        <!--toggle切换-->
        <Col span="10">
                ...
                <li v-for="(item,index) in menuList" class="submenu">
                    <template v-if="item.son">
                        ...
                        <ul class="submenu_item" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
                            <li v-for="child in item.son" v-if="!child.hidden" :key="child.state" class="el-menu-item" style="padding-left: 40px;" :class="$route.path==child.state?'is-active':''" @click="$router.push(child.state)">{{child.name}}
                            </li>
                        </ul>
                    </template>
                    <template v-else>
                        <li class="submenu">
                            <div class="submenu__title " style="padding-left: 20px;height: 56px;line-height: 56px;padding: 0 20px;" :class="$route.path==item.state?'is-active':''" @click="$router.push(item.state)">
                                <Icon :type="item.icon"></Icon>
                            </div>
                        </li>
                    </template>
